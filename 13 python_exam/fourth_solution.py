...
class Rocket(object):
  def __init__(self, rocket_type, fuel_level, launches):
  ...
  def refill(self):
    needed_fuel = self.max_storage - self.fuel_level
    self.fuel_level = self.max_storage
    return needed_fuel

  def get_stats(self):
    return 'name: ' + self.rocket_type + ', fuel: ' + str(self.fuel_level)


class SpaceX(object):
  def __init__(self, fuel_store):
    self.fuel_store = fuel_store
    self.rockets = []
  ...
  def get_stats(self):
    ...
    return 'rockets: ' + str(len(self.rockets)) + ', fuel: ' + str(self.fuel_storage) + ', launches:' + str(launches)
